...
`<div id="id名"></div>` //需要 class 属性来调用 class 类。
...
盒子模型：就是把 HTML 页面布局看作是一个可以装内容的盒子，css 盒子模型用来装饰它。
...
Cutterman，运行在 Photoshop 中的插件 //官网下载 [链接](http://www.cutterman.cn/zh/cutterman)
